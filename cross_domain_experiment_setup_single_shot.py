...
NE_syn = glob.glob(r'C:\Users\Student\Box Sync\Bass Connections 2020-2021\Wind Turbine Object Detection Dataset\Synthetic Imagery\New Distribution\NE\200 synthetic images\*.png')
...
ratios = [[0,100,200], [10, 100, 200], [20, 100, 200]] # In format [[# real, # syn], ...]
pairs = [[EM_train, EM_val], [NE_train, NE_val], [NW_train, NW_val]] # Pairs of regions for each experiment. In format [[training_paths, validation_paths], ...]
pairs_names = [['EM', 'EM'], ['NE', 'NE'], ['NW', 'NW']] # Pairs of regions for each experiment, but in string format
syn_data = [EM_syn, NE_syn, NW_syn] # Synthetic images for each experiment. As we are doing it now, this should match up with the validation region
...
baseline_folder_name = 'baseline_{}'
adding_synthetic_folder_name = 'adding_synthetic_{}'
...
for ratio in ratios:
    for i in range(len(pairs)):
        ...
        baseline_folder = os.path.join(output_folder, baseline_folder_name.format(ratio[0]))
        if not os.path.exists(baseline_folder):
            os.mkdir(baseline_folder)

        adding_synthetic_folder = os.path.join(output_folder, adding_synthetic_folder_name.format(ratio[0]))
        ...
        with open(os.path.join(baseline_folder, baseline_data_filename), 'w') as baseline_data:
            baseline_data.write(f'train={baseline_folder_name.format(ratio[0])}/{training_img_txt_filename}\n')
            baseline_data.write(f'train_label={baseline_folder_name.format(ratio[0])}/{training_lbl_txt_filename}\n')
            baseline_data.write('classes=1\n')
            baseline_data.write(f'valid={baseline_folder_name.format(ratio[0])}/{validation_img_txt_filename}\n')
            baseline_data.write(f'valid_label={baseline_folder_name.format(ratio[0])}/{validation_lbl_txt_filename}\n')
            baseline_data.write(f'names={baseline_folder_name.format(ratio[0])}/wnd.names\n')
        ...
        with open(os.path.join(adding_synthetic_folder, adding_synthetic_data_filename), 'w') as adding_synthetic_data:
            adding_synthetic_data.write(f'train={adding_synthetic_folder_name.format(ratio[0])}/{training_img_txt_filename}\n')
            adding_synthetic_data.write(f'train_label={adding_synthetic_folder_name.format(ratio[0])}/{training_lbl_txt_filename}\n')
            adding_synthetic_data.write('classes=1\n')
            adding_synthetic_data.write(f'valid={adding_synthetic_folder_name.format(ratio[0])}/{validation_img_txt_filename}\n')
            adding_synthetic_data.write(f'valid_label={adding_synthetic_folder_name.format(ratio[0])}/{validation_lbl_txt_filename}\n')
            adding_synthetic_data.write(f'names={adding_synthetic_folder_name.format(ratio[0])}/wnd.names\n')
