...
with open('EM_cluster_train.csv', 'r') as f:
    EM_train = [line.split(',')[1] for line in f.readlines()][1:]

with open('EM_cluster_test.csv', 'r') as f:
    EM_val = [line.split(',')[1] for line in f.readlines()][1:]

with open('NE_cluster_train.csv', 'r') as f:
    NE_train = [line.split(',')[1] for line in f.readlines()][1:]

with open('NE_cluster_test.csv', 'r') as f:
    NE_val = [line.split(',')[1] for line in f.readlines()][1:]

with open('NW_cluster_train.csv', 'r') as f:
    NW_train = [line.split(',')[1] for line in f.readlines()][1:]

with open('NW_cluster_test.csv', 'r') as f:
...
pairs = [[EM_train, NW_val], [NE_train, NW_val], [NW_train, NW_val], 
        [EM_train, NE_val], [NW_train, NE_val], [NE_train, NE_val], 
        [NW_train, EM_val], [NE_train, EM_val], [EM_train, EM_val]] # Pairs of regions for each experiment. In format [[training_paths, validation_paths], ...]
pairs_names = [['EM', 'NW'], ['NE', 'NW'], ['NW', 'NW'],
                ['EM', 'NE'], ['NW', 'NE'], ['NE', 'NE'],
                ['NW', 'EM'], ['NE', 'EM'], ['EM', 'EM']] # Pairs of regions for each experiment, but in string format
syn_data = [NW_syn, NW_syn, NW_syn, NE_syn, NE_syn, NE_syn, EM_syn, EM_syn, EM_syn] # Synthetic images for each experiment. As we are doing it now, this should match up with the validation region
